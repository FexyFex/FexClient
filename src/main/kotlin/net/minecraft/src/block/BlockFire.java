...
public class BlockFire extends Block
{
    ...
    {
        boolean flag = world.getBlockId(i, j - 1, k) == bloodStone.blockID;
    ...
    private void tryToCatchBlockOnFire(World world, int i, int j, int k, int l, Random random)
    {
        ...
        {
            boolean flag = world.getBlockId(i, j, k) == tnt.blockID;
    ...
    {
        if(world.getBlockId(i, j - 1, k) == obsidian.blockID && portal.tryToCreatePortal(world, i, j, k))
        {
            return;
        }
        if(!world.isBlockOpaqueCube(i, j - 1, k) && !func_263_h(world, i, j, k))
    ...
    public void randomDisplayTick(World world, int i, int j, int k, Random random)
    {
        ...
        if(world.isBlockOpaqueCube(i, j - 1, k) || fire.canBlockCatchFire(world, i, j - 1, k))
        {
            for(int l = 0; l < 3; l++)
            {
        ...
        {
            if(fire.canBlockCatchFire(world, i - 1, j, k))
            {
            ...
            if(fire.canBlockCatchFire(world, i, j, k - 1))
            {
            ...
            if(fire.canBlockCatchFire(world, i, j + 1, k))
            {
